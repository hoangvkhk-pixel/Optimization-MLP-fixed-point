...
set "MLP_OUTDIR=gen_mlp_fixedpoint_split300k_qmission3000_H500_cy06"
set "BRANCH_INIT_DIR=init_h500"
set "BRANCH_N_PER_BRANCH=140"
set "LOG_FILE=logs\optimize_fixedpoint_mlp10.log"
call "%~dp0_env.bat"
if errorlevel 1 goto :err
cd /d "%PROJECT_ROOT%"
if not exist "logs" mkdir "logs"

echo [FIXEDPOINT-MLP] Log: %LOG_FILE%
echo [FIXEDPOINT-MLP] Start > "%LOG_FILE%"
...
echo AERO_MODEL_DIR_NORMAL=%AERO_MODEL_DIR_NORMAL%>> "%LOG_FILE%"
echo AERO_MODEL_DIR_DUCK=%AERO_MODEL_DIR_DUCK%>> "%LOG_FILE%"
...
%PYTHON_EXE% src\run_12branches_new20.py --backend mlp --outdir %MLP_OUTDIR% --model-dir %AERO_MODEL_DIR_NORMAL% --cores %OPT_CORES% --init-dir %BRANCH_INIT_DIR% --n-per-branch %BRANCH_N_PER_BRANCH% >> "%LOG_FILE%" 2>&1
if errorlevel 1 goto :err
